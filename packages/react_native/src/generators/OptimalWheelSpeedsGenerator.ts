...
function OptimalWheelSpeedsGenerator(x: number, y: number, maxX: number, maxY: number, powerMode: number): number[] {
    let wheelSpeeds = calculateSpeedCarWheel(x, y, maxX);
...
function getTypeCarDifferentialRatio(y: number) {
    ...
    if (Math.abs(y) > CarJoystickStepPowerEnum.Low && Math.abs(y) <= CarJoystickStepPowerEnum.Medium) {
...
function calculateSpeedCarWheel(x: number, y: number, maxX: number): number[] {
    ...
    if (Math.abs(y) <= CarJoystickStepPowerEnum.Low) {
        wheelSpeeds = calculateAngleCarWheel(carDifferentialRatioEnum.Low, wheelSpeeds, x);
    }

    if (Math.abs(y) > CarJoystickStepPowerEnum.Low && Math.abs(y) <= CarJoystickStepPowerEnum.Medium) {
        wheelSpeeds = calculateAngleCarWheel(carDifferentialRatioEnum.Middle, wheelSpeeds, x);
    }

    if (Math.abs(y) > CarJoystickStepPowerEnum.Medium && Math.abs(y) <= CarJoystickStepPowerEnum.High) {
        wheelSpeeds = calculateAngleCarWheel(carDifferentialRatioEnum.High, wheelSpeeds, x);
    }

    return wheelSpeeds;
}

function calculateAngleCarWheel(carDifferentialRatio: number, wheelSpeeds: number[], x: number): number[] {
    if (Math.sign(x) === 1) {
