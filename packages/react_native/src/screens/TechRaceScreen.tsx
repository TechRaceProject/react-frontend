...
const TechRaceScreen: React.FC = () => {
  ...
  const [sportModePower, setSportModePower] = useState(CarModePowerEnum.EcoModePower);
  const [raceStartedOnce, setRaceStartedOnce] = useState(false); 
  const [showHistoryPopup, setShowHistoryPopup] = useState(false);

  useEffect(() => {
    let [x, y] = AdjustPositionJoystickScreenModeGenerator(joystickPosition, 'portrait');
    console.log('[x, y]', [x, y]);
    ...
    if (socket.readyState === WebSocket.OPEN) {
      ...
      console.log(`Command sent with power: ${sportModePower}`);
    }
  ...
  useEffect(() => {
    if (socket.readyState === WebSocket.OPEN) {
      ...
      socket.onopen = () => {
        setIsConnected(true);
        console.log('WebSocket connected via socket.config.ts');
      };

      socket.onclose = (event) => {
        setIsConnected(false);
        console.log('WebSocket closed via socket.config.ts:', event);
      };
  ...
  const toggleBuzzer = () => {
    toggleBuzzerAction(buzzerActive, setBuzzerActive);  
  };

  const toggleLed = () => {
    toggleLedAction(ledActive, setLedActive);  
  };
  ...
  return (
    <ImageBackground
      ...
      resizeMode="stretch"
    >
      <Text style={[styles.headerText, styles.fontFamily]}>Tech Race</Text>
      ...
      />
        <View style={styles.centerTopCenter}>
          ...
          />
          <Text style={isConnected ? styles.statusTextConnected : styles.statusTextDisconnected}>
            Status de la voiture : <Text style={isConnected ? styles.statusConnected : styles.statusDisconnected}>
      ...
      <View style={styles.rowMiddleCenter}>
        <View style={styles.columnMiddleLeft}>
          ...
          <View style={styles.batteryVolume}>
            ...
            />
            <Text style={styles.batteryText}>38<Text style={styles.batteryPercentage}>%</Text></Text>
          </View>
        ...
        <View style={styles.columnMiddleCenter}>
          <View style={styles.columnTopMiddleCenter}>
            <Text style={[styles.modeTitle, styles.fontFamily]}>MODE:</Text>
      ...
      <CreateRacePopup visible={showPopup} onClose={togglePopup} onSubmit={handleStartRace} />
    </ImageBackground>
  );
};
