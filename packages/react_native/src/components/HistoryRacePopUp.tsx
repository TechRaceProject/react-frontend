...
const RaceHistoryPopup: React.FC<RaceHistoryPopupProps> = ({ visible, onClose }) => {
  ...
  const fetchRaces = async () => {
    try {
      ...
      Alert.alert('Erreur', 'Une erreur est survenue lors de la récupération des courses.');
    }
  };
  ...
  const deleteRace = async (raceId: number) => {
    try {
      const response = await fetch(`http://10.0.2.2:8000/api/races/${raceId}`, {
        method: 'DELETE',
        headers: {
          Authorization:
            'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRlc3RAdGVzdC5jb20iLCJleHAiOjE3MjYxOTkxNzJ9.efdris3dzTMPXr-4aDjjyxa_kuSQ_f9yzxIKXnVDpMk',
      ...
      Alert.alert('Erreur', 'Une erreur est survenue lors de la suppression de la course.');
    }
  };
    ...
    <View style={styles.card}>
      <View style={styles.raceDetails}>
        ...
        <Text style={styles.raceDuration}>
          Durée: {item.start_time && item.end_time ? calculateDuration(item.start_time, item.end_time) : 'Non terminée'}
        </Text>
      </View>
      {/* Delete Button (using Emoji) */}
    ...
    <Modal transparent={true} visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <Text style={styles.modalTitle}>Historique des courses</Text>
          {loading ? (
